...
public class ConstantRecord extends Record {
     ...
     */
    private static ConstantRecord record;
     ...
     * @pre cert
     * @post crea un record de tipus ConstantRecord
     * @param tipus indica el tipus de record
     */
    private ConstantRecord(RecordType tipus) {
        super(tipus);
        this.value = 0;
        this.description = "Jugador amb millor temps mitjà per partida.";
    }
     ...
     */
    public static ConstantRecord getInstance() {
        if (record == null) {
            record = new ConstantRecord(RecordType.CONSTANT);
        }
        return record;
    }
    ...
    public boolean update(RegisteredUser user) {
        ...
        long value = (long) user.getAverageTime();
