 ...
 */
public class CreatorRecord extends Record {

    /**
     * Instancia de la classe
     */
    private static CreatorRecord record;
     ...
     * @pre cert
     * @post crea un record de tipus CreatorRecord
     * @param tipus indica el tipus de record
     */
    private CreatorRecord(RecordType tipus) {
        super(tipus);
        this.value = 0;
        this.description = "Jugador que ha creat més Sudokus.";
    }
     ...
     */
    public static CreatorRecord getInstance() {
        if (record == null) {
            record = new CreatorRecord(RecordType.CREATOR);
        }
        return record;
    }
    ...
    public boolean update(RegisteredUser user) {
        ...
        int value = user.getBoardsCreated();
