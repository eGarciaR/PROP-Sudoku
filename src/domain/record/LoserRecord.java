 ...
 */
public class LoserRecord extends Record {

    /**
     * Instancia de la classe
     */
    private static LoserRecord record;
     ...
     *
     * @pre cert
     * @post crea un record de tipus LoserRecord
     * @param tipus indica el tipus de record
     */
    private LoserRecord(RecordType tipus) {
        super(tipus);
        this.value = 0;
        this.description = "Jugador amb més partides perdudes.";
    }
     ...
     */
    public static LoserRecord getInstance() {
        if (record == null) {
            record = new LoserRecord(RecordType.LOSER);
        }
        return record;
    }
    ...
    public boolean update(RegisteredUser user) {
        ...
        int value = user.getGamesUnfinished();
